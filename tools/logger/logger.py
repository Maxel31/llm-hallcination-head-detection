...
class Logger(logging.Logger):
    ...
    def __init__(
        ...
        if log_type == LogType.GOOGLE_CLOUD:
            ...
            gc_formatter = GoogleCloudFormatter()
            gc_handler = StructuredLogHandler(stream=sys.stdout)

            gc_handler.setFormatter(gc_formatter)
            self.addHandler(gc_handler)
            return

        from tools.logger import LocalFormatter

        local_formatter = LocalFormatter()
